...
def upgrade() -> None:
    ...
    op.execute(sa.text(
        "CREATE INDEX idx_budget_ledger_org_scope ON budget_ledger (org_id, scope, period)"
    ))
    ...
    # ------------------------------------------------------------------
    # Row-level security (FORCE so owner is also subject to policy)
...
def downgrade() -> None:
    ...
    op.execute(sa.text("DROP FUNCTION IF EXISTS skylize_prevent_mutation()"))
    for table in [
        "tenant_integrations",
        "audit_log",
        "kg_edges",
