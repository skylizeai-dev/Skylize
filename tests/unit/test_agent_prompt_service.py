...
def test_known_agent_returns_prompt(svc: AgentPromptService) -> None:
    resp = svc.get_prompt("cfo_agent", org_id="test-org")
    assert resp.agent_id == "cfo_agent"
...
def test_frontier_model_for_vp(svc: AgentPromptService) -> None:
    resp = svc.get_prompt("vp_creative", org_id="test-org")
...
def test_mini_model_for_director(svc: AgentPromptService) -> None:
    resp = svc.get_prompt("copy_director", org_id="test-org")
...
def test_response_max_token_budget(svc: AgentPromptService) -> None:
    resp = svc.get_prompt("cmo", org_id="test-org")
    assert resp.max_token_budget == 100_000
