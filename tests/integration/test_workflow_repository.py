...
async def test_record_step_persists_every_field(app_db, admin_conn) -> None:
    ...
    await admin_conn.execute("DELETE FROM workflow_run_steps WHERE org_id=$1", org)
    await admin_conn.execute("DELETE FROM tenants WHERE org_id=$1", org)
...
async def test_rls_isolates_run_steps_across_tenants(app_db, admin_conn) -> None:
    ...
    await admin_conn.execute("DELETE FROM workflow_run_steps WHERE org_id=ANY($1::text[])",
                             [org_a, org_b])
    await admin_conn.execute("DELETE FROM tenants WHERE org_id=ANY($1::text[])", [org_a, org_b])
