...
**Describes commit:** `4c6f45111fca27a223ca79f08b968f952c914588` on branch `feat/durable-governance` (was `22aeffed` at first generation).
**Revised:** 2026-07-31, merging the findings of three sessions that ran concurrently in one working tree (`8d2055c0`, `be1397aa`, `b5c5813f`, `2b8a2912`, `cd02456a`, `14e959a2`, `38e1818e`, `137c1534`, `4c6f4511`). Two of those sessions were forbidden to edit this file and reported merge text instead; every claim merged from them was re-verified against code before being written here, and the claims that did not survive verification are named in the merge report rather than recorded as fact. One scope claim was corrected on merge (exposure item 17).
...
GATEWAY CONTAINER IMAGE:       exists (ONE); builds and runs locally; never deployed
...
EXPOSURE ITEMS: 17   -- item 9 RESOLVED BY DELETION; 16, 17 added (memory recall)
```
...
|---|---|---|
| `Dockerfile` (root) | **the gateway — and now the only gateway Dockerfile** | Multi-stage. `CMD ["sh","-c","alembic upgrade head && uvicorn skylize.edge.gateway:app --host 0.0.0.0 --port 8000 --workers 1"]` (`:83`). Builder now copies `pyproject.toml` **and** `src` before `pip install --prefix=/install .` (`:29-32`) — the omission that made this image unbuildable is fixed (§10). The dead `PATH=/install/bin` is gone; `COPY --from=builder /install /usr/local` (`:59`) lands console scripts on the existing PATH, and `PYTHONPATH=/app/src` is deliberately **not** set (`:37-44`) so a broken install fails instead of being masked by a loose source copy. Adds `curl` (`:54-56`) because both the `HEALTHCHECK` (`:78-79`) and the ECS container health check invoke it and `python:3.12-slim` ships none. Migrations and `alembic.ini` are copied separately (`:66-67`) since `packages.find` is scoped to `src/`. Built by `deploy-staging.yml:186`, `deploy.ps1:172`, and all three compose build services |
| ~~`infra/Dockerfile`~~ | — | **DELETED (`cd02456a`).** It was the image `docker compose` built while CI and `deploy.ps1` built the root one, so CI validated an image nobody ran and compose ran an image CI never validated. One image now; the root `Dockerfile` header (`:2-8`) records the reconciliation and says not to reintroduce a second |
| `infra/opa/Dockerfile` | dependency only (OPA) | `FROM openpolicyagent/opa:1.18.2`, copies `policy/` (`:12-14`) |
| `infra/docker-compose.yml` | gateway **and** dependencies | `postgres` (`:6-20`), `redis` (`:22-32`), `opa` (`:42-52`), `migrate` (`:56-70`), `gateway` (`:72-105`), `decision-engine-worker` behind `profiles: ["opa-engine"]` (`:130-162`). **All three build services now point at the root `Dockerfile`** (`:62-63`, `:74-75`, `:133-134`) — repointed by `cd02456a`; `:58-61` records why. No qdrant, n8n or temporal service — note that the absent qdrant service is what masks exposure item 16 locally |
...
| `deploy.ps1` | gateway | terraform init/plan/apply (`:126,130,138`), ECR login (`:150`) + `docker build -t $FULL_TAG -f Dockerfile .` (`:171-172`) + push (`:175`), migrations via `aws ecs run-task` (`:243,260`, skippable `:48,242`), force-new-deployment (`:287,291`), `/health` smoke test (`:301`). **Tag is the short commit SHA, not `:latest`** (`:165-167`, `137c1534`); `:157-164` records that `:latest` is restored only once a deploy has gone green end to end |
...
### 10. Gateway container image — **exists (ONE); now built and run locally; never deployed**

**Superseded 2026-07-31 (`cd02456a`).** This section previously read "exists (two, divergent); never built or run". Both halves have changed, and the second changed by measurement rather than by argument.

**The two definitions are now one.** `infra/Dockerfile` is deleted; the root `Dockerfile` is the only gateway image and all three compose build services plus CI and `deploy.ps1` build it (§9).

**E7's UNVERIFIED question is answered, and the answer was the worse branch.** E7 asked whether the root Dockerfile's builder stage — which copied only `pyproject.toml` before `pip install .` while `pyproject.toml:103-104` sets `packages.find where = ["src"]` — would error or "silently emit an empty wheel that `PYTHONPATH=/app/src` masks". It **errors**. setuptools resolves `egg_base` against `src/` and the build aborts:

```
error: error in 'egg_base' option: 'src' does not exist or is not a directory
```

recorded at `Dockerfile:23-28`. So the image CI builds had **never built at all** — the `docker build` step would have failed on its first execution, which is consistent with that step never having run (below). The fix is `COPY src ./src` (`Dockerfile:30`); `PYTHONPATH=/app/src` was *removed* rather than relied on (`Dockerfile:37-44`, and from the task definition at `ecs/main.tf:114-118`), so a future broken install fails loudly instead of being masked by a loose source copy.

**It has now been built and run — locally, by this work.** Three image artefacts exist on the build host from 2026-07-31 (`skylize-infra-audit:test` 14:28:46, `skylize-survivor:test` 14:35:04, `skylize-final:proof` 14:52:59). Running the built image confirms the install is real, not an empty wheel: `skylize` imports from `/usr/local/lib/python3.12/site-packages/skylize` with **206 modules** and `PYTHONPATH` unset, and `alembic`, `uvicorn` and `curl` all resolve inside the container. That is the whole of the claim — a local `docker build` and a local `docker run`. It is **not** evidence of a deployment.

Evidence it has still never been deployed or run as a service:
- No `.tfstate`, no `.terraform/`, no image digest, tag file or registry reference anywhere in the tree.
- The only CI step that builds it (`deploy-staging.yml:186`) sits behind `needs: integration-test` in a workflow that only triggers on `push` to `main` — which these commits have never reached.
- No container has ever been created from any of the three images (`docker ps -a` lists none), so the gateway process itself — `alembic upgrade head && uvicorn ...` against a real database — remains unexercised. The image was proven to *import*, not to *serve*.
...
**The ECS task definition still cannot boot — but for one reason now, not two.** `14e959a2` closed both originally-named blockers:
- `SKYLIZE_JWT_SECRET` — **supplied** (`ecs/main.tf:151-158`) from a new Secrets Manager shell (`secrets/main.tf:48-53`, exported `outputs.tf:8`, passed by *name* rather than by list position — `ecs/main.tf:121-124` warns that `var.secret_arns[N]` is positionally indexed and reordering it silently rewires a secret). The shell is created empty; populating it is an operations step.
- `SKYLIZE_ANTHROPIC_API_KEY` — **no longer required**, because `SKYLIZE_LLM_DEMO_MODE=true` is set explicitly (`ecs/main.tf:89-112`). This satisfies `bootstrap.py`'s fail-closed LLM check without inventing a key. It is a deliberate posture choice, recorded as an owner decision in the file: staging boots in demo mode, where 7 of 21 agents have a canned payload and the rest raise `DemoResponseUnavailable`. Switching to a real provider is a documented four-step change (`:105-111`).

**A third blocker remains, and it was found by the same work that closed the first two** (`ecs/main.tf:161-174`): **`SKYLIZE_APP_DB_PASSWORD` is absent.** The container's CMD is `alembic upgrade head && uvicorn ...`, and migration 0003 reads that variable to decide whether `skylize_app` is created `LOGIN PASSWORD '<pw>'` or a bare `LOGIN` with no password. Nothing in the terraform creates that role — `modules/rds/main.tf` provisions only the `skylize` master user — so the role comes into existence on first boot **with no password**, the `DATABASE_APP_URL` secret then carries a password RDS rejects, and `bootstrap.py` fails at `await db.connect()`. Ordering is fine; the password is not. Closing it needs a `DB_APP_PASSWORD` shell wired both as a task secret and into whatever populates `DATABASE_APP_URL`, so the two agree — deliberately not created in code because it is coupled to a secret value the owner must choose.

So the ALB health check on `/health` (`ecs/main.tf:176-181`) still could never have passed. The count of boot blockers went 2 → 1, and the remaining one is now documented in the file that carries it.
...
- **AWS Secrets Manager** is the only real integration. `infra/terraform/staging/modules/secrets/main.tf:4-45` creates **eight** empty secret shells (`DATABASE_URL` `:4`, `DATABASE_APP_URL` `:10`, `REDIS_URL` `:16`, `HMAC_SECRET` `:22`, `LANGFUSE_SECRET_KEY` `:28`, `GOVERNANCE_SIGNING_KEY_PEM` `:34`, **`JWT_SECRET` `:48` — added by `14e959a2`**, `DB_PASSWORD` `:55`); `DB_PASSWORD` is seeded with a literal placeholder and `ignore_changes`. A ninth, `DB_APP_PASSWORD`, is required and absent (§10). ARNs feed the ECS task `secrets` block (`ecs/main.tf:79-100`). A `secretsmanager` VPC endpoint exists (`vpc/main.tf:107-114`). **Population is a printed reminder in `deploy.ps1:143-165`, not an automated step.**
...
> **This application HAS NEVER run outside a local developer machine.** That verdict is unchanged, but its supporting evidence is now narrower and more precise than when it was written. What changed on 2026-07-31: the gateway image was **built and run locally for the first time** — and only by that build did it emerge that the image CI builds had never built at all, aborting on `egg_base` (§10, E7); one of the two divergent Dockerfiles was deleted; and two of the three ECS boot blockers were closed. What did **not** change: no terraform state and no registry artefact anywhere in the tree; the only image-building CI job still gated behind a `main` push these commits have never made (`deploy-staging.yml:186` behind `needs: integration-test`); the task definition still missing `SKYLIZE_APP_DB_PASSWORD`, which fails the process closed at `await db.connect()` on first boot (`ecs/main.tf:161-174`); no container ever created from any built image; and in-tree operational documents stating it directly (`docs/08_operations/opa_staging_bring_up.md:3,137`, `OVERNIGHT_SESSION_2026-07-21.md:4`). **A local `docker build` plus `docker run` is not a deployment**, and the distance to one is now one secret decision plus a first `main` push, rather than an unknown number of unbuilt-image failures.
...
**The table list above covers Postgres only, and tenant data does not all live in Postgres.** The vector store is a single Qdrant collection, `platform_knowledge` (`memory/qdrant_adapter.py:67`), holding every tenant's points. Qdrant has **no store-level enforcement equivalent to RLS**: there is no policy the database applies regardless of what the query asks for, so the `org_id` payload condition *is* the tenant boundary. `b5c5813f` made that condition structurally unbypassable from the application side (§17), which is the strongest guarantee available without store support — but it is still the application enforcing it. A Qdrant client reaching the collection with any other credential, or a future adapter method that never calls `org_scope.scoped_filters`, is not stopped by the store.

**And the one Postgres table built for tenant memory has RLS but no code.** `memory_records` is created by `0001_initial_schema.py:232`, is in the RLS array (`0001:353`) so it carries `ENABLE` + `FORCE` + a `tenant_isolation` policy, gets the rehydrate carve-out (`0002:47`), is re-granted to the app role on the RLS-subject migration (`0003:38`), and is given full DML plus a uniqueness index by `0005_memory_provenance.py:107-117`. **Nothing in `src/` reads or writes it.** The only occurrence of the name anywhere under `src/` is a docstring line (`schemas/memory.py:65`, "Stored as a JSONB array element on memory_records.provenance") — not a query. Four migrations maintain a table with no reader and no writer; its RLS policy protects nothing today, and its correctness has never been exercised by a live path.
...
| `memory/qdrant_adapter.py:67` — `_COLLECTION = "platform_knowledge"` | **one Qdrant collection for all tenants**; every read/write targets it | **Store-level enforcement is still absent — Qdrant has no RLS equivalent — but as of `b5c5813f` the org scope is structurally required rather than a call-site convention.** `memory/org_scope.py` is the single definition of the rule, driver-free so both stores share it: `require_org` (`:36`) fails closed on a non-string or blank org rather than widening to the whole collection, and `scoped_filters` (`:46`) always emits the `org_id` condition *and rejects a `filters` dict that restates it* (`:53-57`), so a call site cannot shadow, widen, or blank the scope it was handed. `QdrantAdapter._scoped_filter` (`:72-77`) and the in-memory twin (`in_memory.py:18,68`) both build from it, so a test double can no longer prove an isolation guarantee the real store does not make. Residual risk is narrowed from "any existing call site that forgets a filter" to "a future method that never calls the helper" |
...
| E6 | ~~Supply `SKYLIZE_JWT_SECRET` and `SKYLIZE_ANTHROPIC_API_KEY`~~ → **PARTIALLY DONE (`14e959a2`); RESCOPED.** JWT secret wired by name (`ecs/main.tf:151-158` + shell `secrets/main.tf:48`); the Anthropic requirement is retired by setting `SKYLIZE_LLM_DEMO_MODE=true` (`ecs/main.tf:89-112`). **Remaining: `SKYLIZE_APP_DB_PASSWORD`** — absent, and migration 0003 creates `skylize_app` with no password on first boot, so `DATABASE_APP_URL` cannot authenticate (`ecs/main.tf:161-174`) | **SMALL** (mechanically) — **BLOCKED on an owner decision** | the wiring is two entries; the blocker is that the value must agree with whatever populates `DATABASE_APP_URL`, which is a secret the owner chooses. Not creatable in code |
| E7 | ~~Reconcile the two gateway Dockerfiles; confirm the root image actually builds~~ → **DONE (`cd02456a`).** `infra/Dockerfile` deleted; all consumers build the root image; it now builds and the built image runs (§10) | ~~**UNVERIFIED**~~ → **was SMALL** | **The UNVERIFIED question is resolved, and on the worse branch.** setuptools did not silently emit an empty wheel — it **aborted**: `error in 'egg_base' option: 'src' does not exist or is not a directory` (`Dockerfile:23-28`). The image CI builds had never built once. Fixed by `COPY src ./src` (`:30`); `PYTHONPATH=/app/src` was removed rather than leaned on (`:37-44`, `ecs/main.tf:114-118`) so the next broken install fails loudly |
...
### 24. EXPOSURE LIST — claimed but not reachable (17)
...
| 9 | ~~Agent memory — `PgMemoryAdapter` reads and writes `agent_memory_entries`~~ — **RESOLVED BY DELETION (`4c6f4511`)** | `src/skylize/dal/memory_adapter.py` is deleted. The exposure was never the dead code, it was the claim: the repo appeared to have durable agent-memory persistence when it has none. Writing DDL for `agent_memory_entries` would have made an unreachable capability look real in the schema, so the module was removed instead. The orphan allowlist ratchet shrinks honestly, **14 known orphans to 13** (`scripts/orphan_modules.txt`; the `Orphan-module contract` gate reports "13 known, allowlisted"). The `MemoryAdapter` Protocol stays — `MemoryGateway` consumes it and `mem0_adapter` implements it — with its docstring corrected to stop advertising a Postgres adapter that no longer exists. **This resolves the adapter, not the capability: there is still no agent-memory persistence on any path, and `memory_records` still has no reader or writer (§14).** |
...
| 12 | The gateway runs in a container | **narrowed, not closed (`cd02456a`).** There is now one Dockerfile, it builds, and the built image imports the installed package and carries `alembic`/`uvicorn`/`curl` (§10). But no container has ever been created from it (`docker ps -a`: none), so "runs in a container" is proven for `python -c "import skylize"`, not for the gateway serving traffic against a database. Until `14e959a2`'s remaining blocker is closed the ECS task still cannot boot (§10) |
| 13 | Staging exists on AWS ECS (`infra/terraform/staging/`, `deploy.ps1`, `deploy-staging.yml`) | no terraform state anywhere; the task definition omitted **two** variables that each fail the process closed at boot — both closed by `14e959a2`, leaving **one**: `SKYLIZE_APP_DB_PASSWORD`, without which migration 0003 creates `skylize_app` passwordless and `bootstrap.py` fails at `await db.connect()` (`ecs/main.tf:161-174`); `docs/08_operations/opa_staging_bring_up.md:137` — "*ECS path:* **does not exist yet.**" |
...
| 16 | Knowledge search — `GET /api/v1/knowledge/search` is a mounted, documented route (`edge/routes/knowledge.py:232`; mounted `edge/gateway.py:89`) | **it calls a method that does not exist on the installed client.** `QdrantAdapter.search` calls `self._client.search(...)` (`memory/qdrant_adapter.py:233`); `qdrant-client` is **1.18.0** here and `AsyncQdrantClient` exposes **no `search` attribute** — `query_points` replaced it (measured: `hasattr(AsyncQdrantClient, "search")` is `False`, `hasattr(..., "query_points")` is `True`; `pyproject.toml:60` pins only `>=1.9`). The call site already concedes it, carrying `# type: ignore[attr-defined] # qdrant-client API drift` (`:233`), which is also why mypy is silent. The route reaches it directly: `search_knowledge` → `svc.search` (`knowledge.py:240`) → `KnowledgeIngestionService.search` → `self._qdrant.search(...)` (`memory/knowledge_ingestion.py:236`). **Precondition:** `_svc` returns **503** first unless knowledge ingestion is configured (`knowledge.py:104-111`, needs `OPENAI_API_KEY` + `QDRANT_URL`), so the failure is masked in every environment that has not configured Qdrant — including this one. Configure it and the first real query raises `AttributeError`, surfacing as an unhandled 500. `QdrantMemoryRecallPort.recall` (`tools/builtin/memory_recall.py:80`) calls the same dead method. **Not fixed here:** the migration to `query_points` is behavioural (different response shape) and needs a reachable Qdrant to verify |
| 17 | Agent memory recall — `memory.search` is a registered, schema-validated tool (`tools/builtin/memory_recall.py:102`) offered through the governed `ToolProxy` | **it is wired to a null port and always returns zero hits.** `bootstrap.py:439` constructs `default_tool_registry(credential_vault=credential_vault)` and passes **no `memory_recall_port`**, so `build_builtin_tools` falls to `memory_recall_port or NullMemoryRecallPort()` (`tools/builtin/__init__.py:26`), whose `recall` returns `[]` unconditionally (`memory_recall.py:58-61`). `bootstrap.py:436-437` states the intent ("Null ports back memory.search / search.web until real providers are wired"), so the *tool* is honest; the exposure is that an agent asking for prior knowledge gets a successful empty answer, not an error — indistinguishable from "this tenant has no relevant memory". **Scope correction:** it is *not* offered to every agent. Tool exposure is per-contract via `invocable_tools` (`contracts/base.py:88`, subset-checked `:112-117`), and of the 21 MVP contracts only **two** declare any tool — `seo_keyword_agent` → `["search.web", "memory.search"]` (`contracts/mvp/seo.py:29`) and `cfo_agent` → `["utility.current_datetime"]` (`contracts/mvp/finance.py:176`). **Exactly one of 21 agents can invoke `memory.search`** (runtime count over `MVP_REGISTRY`). Combined with item 16, both ends of recall are inert: the port returns nothing, and the real port would call a method that does not exist |
...
(`docs/MVP_GAP_ANALYSIS.md` is tracked as of `edb2d85`. `policy_inputs.md` remains an unapproved DRAFT and stays unstaged — owner decision 1 above.)
