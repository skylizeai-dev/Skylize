...
OPEN DEFECTS: 4          (was 3; #6 added 2026-08-04 -- test_identity.py flake, see entry)
STALE CLAIMS: 14
...
6. **`test_identity.py::test_point_id_is_injective` (`tests/unit/test_identity.py:25`) is flaky under contention — PRIORITY, not deferred.** Mechanism: `hypothesis.errors.FailedHealthCheck` ("Input generation is slow") triggered by the `b_doc` strategy component alone (100% of slow draws both times, ~1.26s and 1.263s; `a_org`/`b_org`/`a_doc` all instant) — a timing health check, not a correctness failure. Reproduced **2/5** in a back-to-back local run against live Postgres/Redis/OPA on `feat/onbehalfof-token` (2026-08-04), with distinct Hypothesis replay seeds each time, so it is the strategy sitting near the health-check threshold, not one bad seed recurring. **This is a distinct fragility from the existing note under CONVENTIONS THAT BIT US** (`.hypothesis/` counterexample caching being worktree-local, item 8) — that note is about a *correctness* replay gap (a stored counterexample not being retried in a fresh worktree); this defect is a *timing* health check unrelated to counterexample storage. Do not conflate the two. **Link to two prior uncaptured occurrences — LIKELY, NOT PROVEN:** PR #8's earlier local run ("1 failed, 1484 passed", test identity never captured before it stopped reproducing) and `feat/cowork-surface`'s `81c17c9` CI flake ("1 failed / 1568 passed", also never captured) are both consistent with this mechanism and both predate this diagnosis, but neither has a captured test identity, so the connection is inference from timing/shape, not confirmation. **Second, independent signal:** `feat/durable-governance`'s post-merge full-suite verification took **371.87s** against an expected ~180s for the equivalent test set — a ~2x slowdown consistent with the same contention sensitivity showing up as elevated wall-clock time even on runs that don't fail outright. **Why priority:** a 40% local flake rate under contention (2/5) is likely to surface in CI's more constrained runners soon, not just in a heavily-loaded local session, and the slow generation may already be costing real wall-clock time on every green run, not only the ones that trip the health check. Not fixed here — candidate fixes (raise the deadline, `suppress_health_check=[HealthCheck.too_slow]`, or simplify the `b_doc` strategy) are an owner decision.
...
15. `src/skylize/bootstrap.py:194-197` — comment "Work journal ... Not yet written to by any live path (the run-path write requires restructuring DeliverableService's transaction boundary ... deferred)". True when `feat/work-journal` landed (its own 3 commits, `75b7b58`/`229e6c3`/`891b68b`, shipped the read side only, and said so accurately). **No longer true**: `feat/cowork-surface` (PR #9, merged `026019e`) added two live, non-transactional writers — `edge/routes/cowork.py:204` (`_journal()` helper, called from the `cowork_turn` POST handler) and `app/hitl/service.py:376` (replay-approval path), both wired with the real `WorkJournal` at `bootstrap.py:289,495`, not a null/test double. The non-transactional design is deliberately documented at both call sites (best-effort, logged-not-raised on failure) — only the `bootstrap.py` comment describing the write path as *absent* was never revised after cowork-surface added it. `GET /api/v1/me/brief` (`edge/routes/brief.py:76`, mounted `gateway.py:94`) is likewise live and reachable, not merely scaffolded.
...
- `SKYLIZE_N8N_API_KEY` empty → `GET /api/v1/agent-prompts/{agent_id}` returns **503** for every caller; the check is NOT disabled (`edge/routes/agent_prompts.py:29-31`). Likewise `SKYLIZE_KNOWLEDGE_WEBHOOK_SECRET` empty → `POST /api/v1/knowledge/ingest` returns **503** (`edge/routes/knowledge.py:139-144`). Both fail closed — verified live against a running gateway.
