...
## 1. Mission
Check assets/copy against brand standards for consistency.
...
## 5. KPIs
Consistency catch rate.

## 6. Inputs
`skylize.schemas.creative.BrandConsistencyAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.BrandConsistencyAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
