...
## 1. Mission
Maintain brand/product positioning relative to market and competitors.

## 2. Responsibilities
- Define positioning statements.
- Track competitive positioning.
- Feed brand/messaging.
...
## 5. KPIs
Positioning clarity; differentiation.

## 6. Inputs
`skylize.schemas.creative.PositioningAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.PositioningAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `brand:positioning`, `market:summary`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
