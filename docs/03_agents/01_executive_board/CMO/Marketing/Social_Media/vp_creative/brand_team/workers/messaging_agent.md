...
## 1. Mission
Craft consistent core messaging for campaigns and brand.

## 2. Responsibilities
- Define message pillars.
- Align copy/creative to messaging.
- Maintain message library.
...
## 4. Escalation Rules
Escalation path: `brand_director > vp_creative > cmo > human_owner`. On a beyond-authority decision or a `fallback_degraded` failure, the Decision Engine routes the proposal to the next entry, emitting `governance.human_escalation_raised` ([agent_governance.md §3](../../../../../../../agent_governance.md#3-authority--escalation)).

## 5. KPIs
Message consistency; resonance.

## 6. Inputs
`skylize.schemas.creative.MessagingAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.MessagingAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `brand:messaging`, `creative:summary`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
