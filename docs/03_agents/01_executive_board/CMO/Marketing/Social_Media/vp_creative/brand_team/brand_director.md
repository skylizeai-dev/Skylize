...
## 1. Mission
Own brand integrity within creative production — direct brand workers and gate brand-sensitive output.

## 2. Responsibilities
- Assign brand consistency/guardian/messaging/positioning/tone work.
- Gate brand-sensitive assets.
...
## 5. KPIs
Brand consistency; brand-incident avoidance.

## 6. Inputs
`skylize.schemas.creative.BrandDirectorIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.BrandDirectorOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `brand:*`, `creative:summary`. **Write:** `brand:approved`
