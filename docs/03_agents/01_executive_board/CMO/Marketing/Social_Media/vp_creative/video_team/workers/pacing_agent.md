...
## 1. Mission
Optimize cut/pacing timing of a video for engagement.

## 2. Responsibilities
- Recommend cut timing.
- Balance pace to content.
- Feed the editor.
...
## 5. KPIs
Pacing acceptance; watch-through.

## 6. Inputs
`skylize.schemas.creative.PacingAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.PacingAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `creative:video:pacing`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
