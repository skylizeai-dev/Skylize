# Agent: `competitor_reels_agent`

**Authority level:** `worker` · **Department:** `creative` · **Escalation path:** `video_director > vp_creative > cmo > human_owner`
...
## 1. Mission
Research competitor short-form video for actionable patterns.
...
## 4. Escalation Rules
Escalation path: `video_director > vp_creative > cmo > human_owner`. On a beyond-authority decision or a `fallback_degraded` failure, the Decision Engine routes the proposal to the next entry, emitting `governance.human_escalation_raised` ([agent_governance.md §3](../../../../../../../agent_governance.md#3-authority--escalation)).

## 5. KPIs
Pattern usefulness; adoption.

## 6. Inputs
`skylize.schemas.creative.CompetitorReelsAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.CompetitorReelsAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
## 10. Events Produced
- `creative.variant_produced` (insights)
...
**Read:** `creative:video:competitor`, `market:summary`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
