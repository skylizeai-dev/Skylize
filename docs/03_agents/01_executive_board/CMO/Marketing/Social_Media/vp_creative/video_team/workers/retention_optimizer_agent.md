# Agent: `retention_optimizer_agent`
...
## 1. Mission
Maximize watch-through by tuning openings and retention hooks.

## 2. Responsibilities
- Analyze drop-off points.
- Recommend retention edits.
- Feed the editor.
...
## 6. Inputs
`skylize.schemas.creative.RetentionOptimizerAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.RetentionOptimizerAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `creative:video:retention`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
