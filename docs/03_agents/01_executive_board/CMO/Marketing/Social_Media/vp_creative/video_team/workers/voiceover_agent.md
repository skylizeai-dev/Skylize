...
## 1. Mission
Produce voiceover audio for video content from a script, on brand. (Stray-text file cleaned per manifest.)

## 2. Responsibilities
- Generate voiceover from script.
- Match tone/pacing to the video.
- Return for QC.
...
## 5. KPIs
Voiceover acceptance; production time.

## 6. Inputs
`skylize.schemas.creative.VoiceoverAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.VoiceoverAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `creative:video:*`, `brand:voice`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
