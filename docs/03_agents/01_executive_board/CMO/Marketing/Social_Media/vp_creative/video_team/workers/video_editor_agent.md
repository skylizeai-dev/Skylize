...
## 1. Mission
Assemble the final video from structure, pacing, retention inputs and assets.

## 2. Responsibilities
- Edit the video to brief.
- Apply structure/pacing/retention notes.
- Return for review.
...
## 5. KPIs
Edit acceptance; production time.

## 6. Inputs
`skylize.schemas.creative.VideoEditorAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.VideoEditorAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `creative:video:*`, `brand:visual`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
