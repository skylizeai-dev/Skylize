...
## 1. Mission
Define the structural beats of a short-form reel for maximum retention.

## 2. Responsibilities
- Lay out hook/build/payoff beats.
...
## 5. KPIs
Structure acceptance; retention.

## 6. Inputs
`skylize.schemas.creative.ReelStructureAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.ReelStructureAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `creative:video:structure`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
