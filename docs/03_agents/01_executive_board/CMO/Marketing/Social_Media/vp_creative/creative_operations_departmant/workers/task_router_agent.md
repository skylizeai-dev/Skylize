...
## 1. Mission
Route incoming creative tasks to the right team/worker based on type, load, and priority. (Stray-text file cleaned per manifest.)

## 2. Responsibilities
- Classify and route creative tasks.
- Balance load across workers.
- Escalate unroutable tasks.
...
## 5. KPIs
Routing accuracy; queue wait time.

## 6. Inputs
`skylize.schemas.creative.TaskRouterAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.TaskRouterAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
