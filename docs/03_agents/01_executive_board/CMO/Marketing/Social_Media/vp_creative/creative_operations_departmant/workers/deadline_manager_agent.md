...
## 1. Mission
Track creative deadlines and flag at-risk items.

## 2. Responsibilities
- Track deadlines per item.
- Flag slippage risk.
- Escalate at-risk work.
...
## 5. KPIs
On-time rate; slippage warning lead time.

## 6. Inputs
`skylize.schemas.creative.DeadlineManagerAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.DeadlineManagerAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `creative:ops:deadlines`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
