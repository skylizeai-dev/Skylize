...
## 1. Mission
Maintain the approved-asset library and serve reusable assets.

## 2. Responsibilities
- Catalog approved assets.
- Serve asset recall.
- Track reuse.
...
## 4. Escalation Rules
Escalation path: `creative_operations_manager > director_social_media > vp_creative > cmo > human_owner`. On a beyond-authority decision or a `fallback_degraded` failure, the Decision Engine routes the proposal to the next entry, emitting `governance.human_escalation_raised` ([agent_governance.md §3](../../../../../../../agent_governance.md#3-authority--escalation)).

## 5. KPIs
Asset findability; reuse rate.

## 6. Inputs
`skylize.schemas.creative.AssetLibraryAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.AssetLibraryAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
## 10. Events Produced
- `memory.write_requested` (asset metadata)
...
**Read:** `creative:ops:assets`. **Write:** `creative:ops:assets`
