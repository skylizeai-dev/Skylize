...
## 1. Mission
Manage the creative approval queue and route items to the right approver.

## 2. Responsibilities
- Track pending approvals.
- Route to approver by authority.
...
## 5. KPIs
Approval cycle time; stale-item rate.

## 6. Inputs
`skylize.schemas.creative.ApprovalManagerAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.ApprovalManagerAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `creative:ops:approvals`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
