# Agent: `visual_consistency_agent`
...
## 1. Mission
Ensure visual consistency across an asset set/campaign.

## 2. Responsibilities
- Compare assets for consistency.
- Flag mismatches.
- Recommend fixes.
...
## 5. KPIs
Consistency score; mismatch catch rate.

## 6. Inputs
`skylize.schemas.creative.VisualConsistencyAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.VisualConsistencyAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
