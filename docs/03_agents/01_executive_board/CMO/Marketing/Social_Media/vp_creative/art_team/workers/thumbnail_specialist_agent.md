# Agent: `thumbnail_specialist_agent`
...
## 1. Mission
Produce high-CTR thumbnails for video content.
...
## 5. KPIs
Thumbnail CTR lift; acceptance.

## 6. Inputs
`skylize.schemas.creative.ThumbnailSpecialistAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.ThumbnailSpecialistAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `creative:art:thumbnails`, `brand:visual`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
