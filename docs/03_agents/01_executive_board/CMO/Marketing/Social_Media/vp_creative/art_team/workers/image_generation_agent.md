...
## 1. Mission
Generate still visual assets for a brief via the image tool, on brand.

## 2. Responsibilities
- Produce candidate images.
- Apply brand style.
- Return options for review.
...
## 5. KPIs
Image acceptance; brand-fit score.

## 6. Inputs
`skylize.schemas.creative.ImageGenerationAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.ImageGenerationAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `creative:art:images`, `brand:visual`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
