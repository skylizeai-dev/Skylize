...
## 1. Mission
Write narrative/long-form story copy for brand and content.

## 2. Responsibilities
- Draft story-format copy.
- Maintain narrative arc and voice.
- Feed content programs.
...
## 5. KPIs
Story acceptance; engagement/dwell.

## 6. Inputs
`skylize.schemas.creative.StoryWriterAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.StoryWriterAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `creative:copy:stories`, `brand:voice`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
