...
## 1. Mission
Write conversion-focused ad copy bodies for a brief and chosen hook.

## 2. Responsibilities
- Draft ad copy variants.
- Align to offer and brand voice.
...
## 5. KPIs
Ad copy acceptance; ad CTR/CVR.

## 6. Inputs
`skylize.schemas.creative.AdCopyAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.AdCopyAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
`allowed_tools`: `llm.generate`, `memory.search`. Token `scope` ⊆ `allowed_tools`, validated signature → expiry → revocation → scope → budget → delegation. `governance_token_required = true`. `max_token_budget = 10000`, `max_execution_time_seconds = 90`, `budget_source = "contract"` (the agent's registered AgentContract). `human_in_loop_triggers`: none (bounded task).

## 12. Memory Requirements
**Read:** `creative:copy:*`, `brand:voice`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
