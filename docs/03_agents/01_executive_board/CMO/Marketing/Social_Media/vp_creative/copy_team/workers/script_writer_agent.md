...
## 1. Mission
Write short-form video/ad scripts for the video team.

## 2. Responsibilities
- Draft scripts to a brief and duration.
...
## 5. KPIs
Script acceptance; retention of resulting video.

## 6. Inputs
`skylize.schemas.creative.ScriptWriterAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.ScriptWriterAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
`allowed_tools`: `llm.generate`, `memory.search`. Token `scope` ⊆ `allowed_tools`, validated signature → expiry → revocation → scope → budget → delegation. `governance_token_required = true`. `max_token_budget = 12000`, `max_execution_time_seconds = 120`, `budget_source = "contract"` (the agent's registered AgentContract). `human_in_loop_triggers`: none (bounded task).

## 12. Memory Requirements
**Read:** `creative:copy:scripts`, `brand:voice`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
