...
## 1. Mission
Write platform-native social captions for approved creative.

## 2. Responsibilities
- Draft captions per platform.
- Match tone and length norms.
- Include CTA where briefed.
...
## 5. KPIs
Caption acceptance; engagement.

## 6. Inputs
`skylize.schemas.creative.CaptionWriterAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.CaptionWriterAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `creative:copy:captions`, `brand:voice`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
