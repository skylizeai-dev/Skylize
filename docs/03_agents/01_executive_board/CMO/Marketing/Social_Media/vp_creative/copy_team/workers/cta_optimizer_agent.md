...
## 1. Mission
Generate and optimize calls-to-action for conversion.

## 2. Responsibilities
- Produce CTA variants.
- Align to offer/funnel stage.
- Rank by expected conversion.
...
## 5. KPIs
CTA click/conversion lift.

## 6. Inputs
`skylize.schemas.creative.CtaOptimizerAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.CtaOptimizerAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
`allowed_tools`: `llm.generate`, `memory.search`. Token `scope` ⊆ `allowed_tools`, validated signature → expiry → revocation → scope → budget → delegation. `governance_token_required = true`. `max_token_budget = 4000`, `max_execution_time_seconds = 45`, `budget_source = "contract"` (the agent's registered AgentContract). `human_in_loop_triggers`: none (bounded task).

## 12. Memory Requirements
**Read:** `creative:copy:cta`, `brand:voice`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
