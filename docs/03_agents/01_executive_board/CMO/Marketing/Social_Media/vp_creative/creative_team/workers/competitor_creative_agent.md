# Agent: `competitor_creative_agent`
...
## 1. Mission
Track competitor creative for benchmarking and gaps.

## 2. Responsibilities
- Analyze competitor creative.
- Benchmark against ours.
- Surface gaps/opportunities.
...
## 4. Escalation Rules
Escalation path: `creative_director > vp_creative > cmo > human_owner`. On a beyond-authority decision or a `fallback_degraded` failure, the Decision Engine routes the proposal to the next entry, emitting `governance.human_escalation_raised` ([agent_governance.md §3](../../../../../../../agent_governance.md#3-authority--escalation)).

## 5. KPIs
Benchmark usefulness; opportunity hit-rate.

## 6. Inputs
`skylize.schemas.creative.CompetitorCreativeAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.CompetitorCreativeAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
## 10. Events Produced
- `creative.review_requested` (insights)
...
**Read:** `creative:competitor:*`, `market:summary`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
