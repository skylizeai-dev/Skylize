...
## 1. Mission
Find emerging viral patterns and trends to inform creative.

## 2. Responsibilities
- Scan for viral patterns.
- Extract transferable mechanics.
- Feed the creative director.
...
## 5. KPIs
Trend timeliness; adoption value.

## 6. Inputs
`skylize.schemas.creative.ViralResearchAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.ViralResearchAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `creative:research:*`, `market:summary`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
