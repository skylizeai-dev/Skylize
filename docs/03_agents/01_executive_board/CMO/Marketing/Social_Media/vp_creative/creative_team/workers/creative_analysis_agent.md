...
## 1. Mission
Analyze why creatives win or lose using performance data.

## 2. Responsibilities
- Correlate creative features to performance.
- Surface drivers.
- Feed direction.
...
## 5. KPIs
Driver accuracy; insight usefulness.

## 6. Inputs
`skylize.schemas.creative.CreativeAnalysisAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.CreativeAnalysisAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
## 10. Events Produced
- `creative.review_requested` (analysis)
...
**Read:** `creative:*`, `campaign:*`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
