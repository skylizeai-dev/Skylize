...
## 1. Mission
Final creative QC across copy+visual+video before approval.
...
## 6. Inputs
`skylize.schemas.creative.QualityControlAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.creative.QualityControlAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `creative:*`, `brand:*`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
