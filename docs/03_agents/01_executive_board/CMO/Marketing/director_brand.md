...
## 1. Mission
Own brand strategy and consistency across all outbound work.

## 2. Responsibilities
- Set brand guidelines.
- Gate brand-sensitive launches.
- Coordinate with creative brand team.
...
## 5. KPIs
Brand consistency score; brand-incident avoidance.

## 6. Inputs
`skylize.schemas.marketing.DirectorBrandIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.marketing.DirectorBrandOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `brand:*`, `marketing:summary`. **Write:** `brand:guidelines`
