...
`allowed_tools`: `llm.generate`, `memory.search`, `bi.query`, `orchestrator.delegate`. Token `scope` ⊆ `allowed_tools`, validated signature → expiry → revocation → scope → budget → delegation. `governance_token_required = true`. `max_token_budget = 120000`, `max_execution_time_seconds = 600`, `budget_source = "contract"` (the agent's registered AgentContract). `human_in_loop_triggers`: `SPEND_OVER_CEILING`, `LOW_CONFIDENCE_IRREVERSIBLE`.
