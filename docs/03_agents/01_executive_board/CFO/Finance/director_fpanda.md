...
## 1. Mission
Own financial planning & analysis — forecasts, variance analysis, and the profitability picture that informs allocation. (Path `director_fpanda` = FP&A.)

## 2. Responsibilities
- Produce and maintain forecasts.
- Run variance and profitability analysis.
- Feed planning inputs to capital allocation.
...
## 5. KPIs
Forecast accuracy; variance explained; planning cycle time.

## 6. Inputs
`skylize.schemas.finance.DirectorFpandaIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.finance.DirectorFpandaOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `finance:fpa:*`, `finance:summary`. **Write:** `finance:fpa:reports`
