# Agent: `director_competitive_intelligence`
...
## 1. Mission
Own competitive intelligence — track competitors and threats.
...
## 5. KPIs
Threat timeliness; insight value.

## 6. Inputs
`skylize.schemas.strategy.DirectorCompetitiveIntelligenceIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.strategy.DirectorCompetitiveIntelligenceOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `strategy:competitive:*`, `market:*`. **Write:** `strategy:competitive:reports`
