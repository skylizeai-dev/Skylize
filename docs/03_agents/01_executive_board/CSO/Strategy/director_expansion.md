...
## 1. Mission
Own market/geographic/segment expansion analysis.

## 2. Responsibilities
- Identify expansion opportunities.
- Model expansion economics.
- Recommend moves.
...
## 5. KPIs
Expansion opportunity quality; outcome.

## 6. Inputs
`skylize.schemas.strategy.DirectorExpansionIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.strategy.DirectorExpansionOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `strategy:expansion:*`, `market:*`. **Write:** `strategy:expansion:plans`
