# Agent: `director_market_intelligence`
...
## 1. Mission
Own market intelligence — trends, sizing, and demand signals.

## 2. Responsibilities
- Track market trends and sizing.
- Surface demand signals.
- Feed strategy/product.
...
## 5. KPIs
Market-insight timeliness/accuracy.

## 6. Inputs
`skylize.schemas.strategy.DirectorMarketIntelligenceIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.strategy.DirectorMarketIntelligenceOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `strategy:market:*`, `market:*`. **Write:** `strategy:market:reports`
