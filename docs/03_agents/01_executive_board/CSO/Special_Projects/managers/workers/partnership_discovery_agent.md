# Agent: `partnership_discovery_agent`
...
## 1. Mission
Discover strategic partnership/venture opportunities.

## 2. Responsibilities
- Find partnership candidates.
- Assess strategic fit.
- Feed ventures/strategy.
...
## 5. KPIs
Candidate quality; fit accuracy.

## 6. Inputs
`skylize.schemas.special_projects.PartnershipDiscoveryAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.special_projects.PartnershipDiscoveryAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `special_projects:*`, `sales:partnerships:summary`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
