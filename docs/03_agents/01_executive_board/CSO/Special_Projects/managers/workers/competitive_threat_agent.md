# Agent: `competitive_threat_agent`
...
## 1. Mission
Detect and assess competitive threats for special projects.
...
## 5. KPIs
Threat detection timeliness.

## 6. Inputs
`skylize.schemas.special_projects.CompetitiveThreatAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.special_projects.CompetitiveThreatAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `special_projects:*`, `market:summary`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
