...
## 1. Mission
Analyze startups as acquisition/partnership/competitive subjects.
...
## 5. KPIs
Analysis accuracy; relevance.

## 6. Inputs
`skylize.schemas.special_projects.StartupAnalysisAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.special_projects.StartupAnalysisAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
