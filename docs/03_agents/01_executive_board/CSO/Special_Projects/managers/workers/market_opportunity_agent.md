...
## 1. Mission
Size and qualify new market opportunities.

## 2. Responsibilities
- Identify opportunities.
- Size and qualify them.
- Feed ventures/expansion.
...
## 5. KPIs
Opportunity quality; sizing accuracy.

## 6. Inputs
`skylize.schemas.special_projects.MarketOpportunityAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.special_projects.MarketOpportunityAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `special_projects:ventures:*`, `market:*`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
