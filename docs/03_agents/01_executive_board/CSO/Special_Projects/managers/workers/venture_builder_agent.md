...
## 1. Mission
Build out incubated ventures from concept toward operation.

## 2. Responsibilities
- Stand up venture plans.
...
## 5. KPIs
Venture build progress; readiness.

## 6. Inputs
`skylize.schemas.special_projects.VentureBuilderAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.special_projects.VentureBuilderAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `special_projects:ventures:*`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
