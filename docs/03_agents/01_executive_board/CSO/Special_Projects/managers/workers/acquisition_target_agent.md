# Agent: `acquisition_target_agent`
...
## 1. Mission
Identify and profile acquisition targets.

## 2. Responsibilities
- Find candidate targets.
- Profile fit/economics.
- Feed the M&A director.
...
## 5. KPIs
Target quality; coverage.

## 6. Inputs
`skylize.schemas.special_projects.AcquisitionTargetAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.special_projects.AcquisitionTargetAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `special_projects:mna:*`, `market:summary`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
