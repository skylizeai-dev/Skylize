...
## 1. Mission
Research high-risk/high-reward moonshot ideas.

## 2. Responsibilities
- Research moonshot concepts.
- Assess feasibility/upside.
- Feed skunkworks.
...
## 5. KPIs
Idea quality; feasibility accuracy.

## 6. Inputs
`skylize.schemas.special_projects.MoonshotResearchAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.special_projects.MoonshotResearchAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `special_projects:skunkworks:*`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
