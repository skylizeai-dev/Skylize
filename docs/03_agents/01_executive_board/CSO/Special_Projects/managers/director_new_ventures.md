...
## 1. Mission
Incubate new business lines as governed ventures. (Depth note: sits under `managers/`; manifest authority = director.)

## 2. Responsibilities
- Identify and incubate new ventures.
- Bound venture risk/spend.
- Promote/sunset ventures.
...
## 5. KPIs
Venture viability; learning per spend.

## 6. Inputs
`skylize.schemas.special_projects.DirectorNewVenturesIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.special_projects.DirectorNewVenturesOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
