...
## 1. Mission
Own user research — validate personas and surface needs.

## 2. Responsibilities
- Run user research.
- Validate personas.
- Feed product priorities.
...
## 5. KPIs
Research coverage; insight adoption.

## 6. Inputs
`skylize.schemas.product.DirectorUserResearchIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.product.DirectorUserResearchOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `product:research:*`. **Write:** `product:research:findings`
