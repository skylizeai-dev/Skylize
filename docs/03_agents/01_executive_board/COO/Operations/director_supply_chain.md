...
## 1. Mission
Own supply-chain planning and inventory risk.

## 2. Responsibilities
- Plan inventory/supply.
- Manage stockout/overstock risk.
...
## 5. KPIs
Stockout rate; inventory turns; supply risk.

## 6. Inputs
`skylize.schemas.operations.DirectorSupplyChainIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.operations.DirectorSupplyChainOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `operations:supply:*`, `procurement:summary`. **Write:** `operations:supply:plans`
