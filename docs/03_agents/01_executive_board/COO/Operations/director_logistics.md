...
## 1. Mission
Own logistics and fulfillment reliability.

## 2. Responsibilities
- Plan logistics/fulfillment.
- Track delivery SLAs.
...
## 5. KPIs
Delivery SLA; logistics cost.

## 6. Inputs
`skylize.schemas.operations.DirectorLogisticsIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.operations.DirectorLogisticsOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `operations:logistics:*`. **Write:** `operations:logistics:plans`
