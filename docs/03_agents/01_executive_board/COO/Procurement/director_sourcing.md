...
## 1. Mission
Own sourcing strategy — find and qualify suppliers cost-effectively.

## 2. Responsibilities
- Run sourcing for needs.
- Qualify suppliers.
- Feed vendor evaluation.
...
## 5. KPIs
Sourcing cost; supplier quality; cycle time.

## 6. Inputs
`skylize.schemas.procurement.DirectorSourcingIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.procurement.DirectorSourcingOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `procurement:sourcing:*`. **Write:** `procurement:sourcing:plans`
