...
## 6. Inputs
`skylize.schemas.procurement.DirectorVendorManagementIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.procurement.DirectorVendorManagementOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
