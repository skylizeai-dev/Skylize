# Agent: `procurement_optimizer_agent`
...
## 1. Mission
Optimize the overall procurement mix (vendors, terms, timing).

## 2. Responsibilities
- Model procurement trade-offs.
...
## 4. Escalation Rules
Escalation path: `manager_procurement_operations > director_contract_procurement > vc_procurement > coo > human_owner`. On a beyond-authority decision or a `fallback_degraded` failure, the Decision Engine routes the proposal to the next entry, emitting `governance.human_escalation_raised` ([agent_governance.md §3](../../../../../agent_governance.md#3-authority--escalation)).

## 5. KPIs
Mix efficiency; recommendation adoption.

## 6. Inputs
`skylize.schemas.procurement.ProcurementOptimizerAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.procurement.ProcurementOptimizerAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `procurement:*`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
