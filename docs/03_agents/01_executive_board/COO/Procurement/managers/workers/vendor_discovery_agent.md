...
## 1. Mission
Discover candidate vendors for a sourcing need.

## 2. Responsibilities
- Search for candidate vendors.
- Pre-qualify by basic criteria.
- Feed evaluation.
...
## 5. KPIs
Candidate quality; discovery coverage.

## 6. Inputs
`skylize.schemas.procurement.VendorDiscoveryAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.procurement.VendorDiscoveryAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `procurement:sourcing:*`, `market:summary`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
