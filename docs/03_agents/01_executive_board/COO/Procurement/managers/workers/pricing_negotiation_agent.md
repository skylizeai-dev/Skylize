# Agent: `pricing_negotiation_agent`
...
## 1. Mission
Draft negotiation strategies and target prices for vendor deals.

## 2. Responsibilities
- Compute target prices.
- Draft negotiation strategy.
- Feed contract procurement.
...
## 5. KPIs
Negotiated savings vs. target.

## 6. Inputs
`skylize.schemas.procurement.PricingNegotiationAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.procurement.PricingNegotiationAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
