...
## 1. Mission
Evaluate candidate vendors against requirements.

## 2. Responsibilities
- Assess vendors on criteria.
- Produce evaluation reports.
- Feed scoring/selection.
...
## 5. KPIs
Evaluation accuracy; selection quality.

## 6. Inputs
`skylize.schemas.procurement.VendorEvaluationAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.procurement.VendorEvaluationAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
