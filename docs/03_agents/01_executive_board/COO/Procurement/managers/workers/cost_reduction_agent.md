...
## 1. Mission
Identify procurement cost-reduction opportunities.

## 2. Responsibilities
- Analyze spend for savings.
...
## 5. KPIs
Realized savings; proposal accuracy.

## 6. Inputs
`skylize.schemas.procurement.CostReductionAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.procurement.CostReductionAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `procurement:*`, `finance:summary`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
