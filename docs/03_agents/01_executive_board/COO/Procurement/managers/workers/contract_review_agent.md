...
## 1. Mission
Review procurement contract terms for risk before commitment.

## 2. Responsibilities
- Parse contract terms.
- Flag risky clauses.
- Return review with risk score.
...
## 5. KPIs
Risk-clause catch rate.

## 6. Inputs
`skylize.schemas.procurement.ContractReviewAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.procurement.ContractReviewAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `procurement:contracts:*`, `legal:summary`. **Write:** none — proposes via `memory.write_requested`; the Memory service persists (workers do not write stores directly).
