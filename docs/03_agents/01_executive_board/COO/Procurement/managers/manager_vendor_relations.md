# Agent: `manager_vendor_relations`
...
## 1. Mission
Manage ongoing vendor relationships and performance.

## 2. Responsibilities
- Track vendor performance.
- Manage relationship issues.
- Feed vendor risk/scoring.
...
## 5. KPIs
Vendor performance; relationship health.

## 6. Inputs
`skylize.schemas.procurement.ManagerVendorRelationsIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.procurement.ManagerVendorRelationsOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `procurement:vendors:*`. **Write:** `procurement:vendors:relations`
