...
`allowed_tools`: `llm.generate`. Token `scope` ⊆ `allowed_tools`, validated signature → expiry → revocation → scope → budget → delegation. `governance_token_required = true`. `max_token_budget = 20000`, `max_execution_time_seconds = 120`, `budget_source = "contract"` (the agent's registered AgentContract). `human_in_loop_triggers`: `SECURITY_SEVERITY_HIGH`.
