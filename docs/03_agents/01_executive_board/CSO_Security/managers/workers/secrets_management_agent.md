# Agent: `secrets_management_agent`
...
## 1. Mission
Monitor for secret exposure and enforce that no secret appears in code/logs/events/memory.

## 2. Responsibilities
- Scan for secret exposure.
- Flag/contain leaks.
- Verify secret-handling policy.
...
## 5. KPIs
Secret-leak catch rate; zero exposed secrets.

## 6. Inputs
`skylize.schemas.security.SecretsManagementAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.security.SecretsManagementAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `security:secrets:*`. **Write:** `security:fraud:signals`
