...
## 1. Mission
Detect security incidents from telemetry and raise them to IR.

## 2. Responsibilities
- Monitor telemetry for incident signatures.
- Raise incidents with severity.
- Feed IR.
...
## 5. KPIs
Detection precision/recall; lead time.

## 6. Inputs
`skylize.schemas.security.IncidentDetectionAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.security.IncidentDetectionAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `security:incident:*`, `security:ops:*`. **Write:** `security:fraud:signals`
