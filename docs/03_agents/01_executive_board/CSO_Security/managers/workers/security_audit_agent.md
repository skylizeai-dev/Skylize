...
## 1. Mission
Continuously audit actions against the immutable audit trail for policy violations.

## 2. Responsibilities
- Audit actions vs. policy.
- Flag violations.
- Feed compliance/IR.
...
## 5. KPIs
Violation catch rate; audit coverage.

## 6. Inputs
`skylize.schemas.security.SecurityAuditAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.security.SecurityAuditAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `security:compliance:*`, `security:ops:*`. **Write:** `security:fraud:signals`
