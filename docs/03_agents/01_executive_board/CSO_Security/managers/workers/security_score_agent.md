...
## 1. Mission
Risk-rank security flags deterministically (Security Score) for triage.

## 2. Responsibilities
- Compute Security Score (severity, blast radius, confidence).
- Rank flags.
- Feed IR/CSO triage.
...
## 5. KPIs
Triage accuracy; severity calibration.

## 6. Inputs
`skylize.schemas.security.SecurityScoreAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.security.SecurityScoreAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `security:*`. **Write:** `security:fraud:signals`

## 13. Success Metrics
Outputs accepted by its parent/Decision Engine; SLOs met; no scope or budget violations; full audit trail.

## 14. Failure Conditions
`failure_mode = fail_closed`. `fail_closed`: returns no score rather than a guess; missing score escalates rather than under-rates a threat.
