...
## 1. Mission
Detect financial-risk/abuse signals from a security lens, complementing finance risk.

## 2. Responsibilities
- Detect financial abuse/anomalies.
- Flag to risk/fraud.
- Veto unsafe financial actions.
...
## 5. KPIs
Financial-abuse catch rate; loss avoided.

## 6. Inputs
`skylize.schemas.security.FinancialRiskAgentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.security.FinancialRiskAgentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `security:fraud:*`, `finance:summary`. **Write:** `security:fraud:signals`
