...
## 6. Inputs
`skylize.schemas.security.DirectorComplianceIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.security.DirectorComplianceOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
