# Agent: `manager_security_operations`
...
## 1. Mission
Run security operations — monitor signals and route detections to workers/IR.

## 2. Responsibilities
- Monitor security signals (`audit.access_denied`, verdicts).
- Route detections.
- Escalate to IR/CSO.
...
## 5. KPIs
Detection routing time; false-positive rate.

## 6. Inputs
`skylize.schemas.security.ManagerSecurityOperationsIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.security.ManagerSecurityOperationsOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `security:ops:*`. **Write:** `security:ops:signals`
