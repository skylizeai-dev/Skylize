...
## 1. Mission
Own retention and churn reduction.

## 2. Responsibilities
- Identify churn risk.
- Drive retention programs.
- Feed retention scoring.
...
## 5. KPIs
Churn reduction; retention rate.

## 6. Inputs
`skylize.schemas.customer_success.DirectorRetentionIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.customer_success.DirectorRetentionOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `customer_success:retention:*`. **Write:** `customer_success:retention:programs`
