...
## 1. Mission
Own customer support quality and resolution.

## 2. Responsibilities
- Govern support workflows.
- Track resolution SLAs/CSAT.
...
## 5. KPIs
Resolution time; CSAT; deflection.

## 6. Inputs
`skylize.schemas.customer_success.DirectorSupportIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.customer_success.DirectorSupportOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `customer_success:support:*`. **Write:** `customer_success:support:playbooks`
