# Agent: `director_account_management`
...
## 1. Mission
Own account growth and expansion revenue.
...
## 5. KPIs
Expansion revenue; account health.

## 6. Inputs
`skylize.schemas.sales.DirectorAccountManagementIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.sales.DirectorAccountManagementOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `sales:accounts:*`, `customer_success:summary`. **Write:** `sales:accounts:plans`
