...
## 1. Mission
Own the Next.js/TS frontend — the owner/operator control plane and dashboards.

## 2. Responsibilities
- Govern frontend architecture.
- Own the control-plane UX (HITL, kill switch, dashboards).
- Uphold TS-strict standards.
...
## 5. KPIs
Frontend reliability; UX quality; accessibility.

## 6. Inputs
`skylize.schemas.engineering.DirectorFrontendIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.engineering.DirectorFrontendOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `engineering:frontend:*`, `product:summary`. **Write:** `engineering:frontend:standards`
