...
## 1. Mission
Own analytics — define metrics, features, and the analytics layer feeding scoring and dashboards.

## 2. Responsibilities
- Define metrics and scoring features.
- Own analytics pipelines.
- Feed BI/scoring.
...
## 5. KPIs
Metric correctness; feature freshness.

## 6. Inputs
`skylize.schemas.data.DirectorAnalyticsIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.data.DirectorAnalyticsOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `data:analytics:*`. **Write:** `data:analytics:metrics`
