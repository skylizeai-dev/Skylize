# Agent: `director_business_intelligence`
...
## 1. Mission
Own BI — dashboards and the projected organizational-memory reporting layer.

## 2. Responsibilities
- Own per-tenant dashboards.
- Project decision/outcome reporting.
- Serve `bi.query`.
...
## 5. KPIs
BI timeliness; dashboard usefulness.

## 6. Inputs
`skylize.schemas.data.DirectorBusinessIntelligenceIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.data.DirectorBusinessIntelligenceOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `data:bi:*`, `org:decisions`. **Write:** `data:bi:reports`
