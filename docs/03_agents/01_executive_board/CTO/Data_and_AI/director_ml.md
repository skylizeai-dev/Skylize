...
## 1. Mission
Own ML models — scoring/re-ranking models and the governed, tenant-isolated learning pipeline.

## 2. Responsibilities
- Implement scoring/re-ranking models (explainable).
- Own the learning pipeline with privacy/security gates.
- Register/evaluate model versions.
...
## 5. KPIs
Model lift; explainability coverage; zero isolation violations.

## 6. Inputs
`skylize.schemas.data.DirectorMlIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.data.DirectorMlOut` — its produced artifact, wrapped by the Orchestrator into the correct event.

## 8. Dependencies
scoring models; the learning pipeline; `director_privacy`/`chief_security_officer` gates.
...
**Read:** `data:ml:*`. **Write:** `data:ml:models`
