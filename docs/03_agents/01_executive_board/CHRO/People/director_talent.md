...
## 1. Mission
Own agent capability portfolio — which agent roles exist, gaps, and onboarding of new agents/departments.

## 2. Responsibilities
- Maintain the capability/role map.
- Plan new-agent onboarding with playbooks.
...
## 5. KPIs
Capability coverage; onboarding time; gap closure.

## 6. Inputs
`skylize.schemas.people.DirectorTalentIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.people.DirectorTalentOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `people:talent:*`, `org:playbooks`. **Write:** `people:talent:plan`
