...
## 1. Mission
Govern agent performance review — measure agents against KPIs and drive improvement loops.

## 2. Responsibilities
- Define and run agent performance reviews.
- Surface under-performers for retraining.
- Feed performance into capability planning.
...
## 5. KPIs
Performance coverage; improvement realized; KPI attainment.

## 6. Inputs
`skylize.schemas.people.DirectorPerformanceIn` — the scoped work item it consumes (validated against its contract `input_schema`).

## 7. Outputs
`skylize.schemas.people.DirectorPerformanceOut` — its produced artifact, wrapped by the Orchestrator into the correct event.
...
**Read:** `people:performance:*`, `org:summary`. **Write:** `people:performance:reviews`
