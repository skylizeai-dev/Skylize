...
# Tracking: docs/04_decision_engine/guardrails.md §3 (policy class: spend) ·
...
# the "spend" question — "is the amount within tenant/department/campaign
# ceilings?" — with an unconditional DENY. Fail-closed until authored.
package skylize.decision.spend

# Default deny — absence of an explicit allow is a denial (guardrails.md §4).
# No rule below sets `allow := true`, so this class can never approve any spend.
default allow := false
...
deny_reasons contains "PLACEHOLDER spend policy: no ceilings authored — fail-closed default deny (see docs/04_decision_engine/guardrails.md §3)"
