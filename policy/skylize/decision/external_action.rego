...
# Tracking: docs/04_decision_engine/guardrails.md §3 (policy class: external_action) ·
...
# the "external_action" question — "is this a first launch / new channel / new ad
# account?" — with an unconditional DENY. Fail-closed until authored.
package skylize.decision.external_action

# Default deny — absence of an explicit allow is a denial (guardrails.md §4).
# No rule below sets `allow := true`, so no external action can be approved.
default allow := false
...
deny_reasons contains "PLACEHOLDER external_action policy: no rule authored — fail-closed default deny (see docs/04_decision_engine/guardrails.md §3)"
