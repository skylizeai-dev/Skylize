...
# Tracking: docs/04_decision_engine/guardrails.md §3 (policy class: data_access) ·
...
# the "data_access" question — "is the data access cross-tenant or out-of-scope?"
# — with an unconditional DENY. Fail-closed until authored.
package skylize.decision.data_access

# Default deny — absence of an explicit allow is a denial (guardrails.md §4).
# No rule below sets `allow := true`, so no data access can be granted here.
default allow := false
...
deny_reasons contains "PLACEHOLDER data_access policy: no rule authored — fail-closed default deny (see docs/04_decision_engine/guardrails.md §3)"
