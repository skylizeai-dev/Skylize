...
// The console app consumes the SAME rows with the TS types stripped. It is a
// generated file too -- never hand-edit it. CI diffs BOTH outputs against a
// fresh run of this script (scripts/check_agent_network_data.py).
const OUT_CONSOLE = "app/Skylize-Console-Black/src/data/agentNetworkData.js";
...
// Palette note: 18 hues distributed around the wheel at muted saturation
...
/* ── department overrides: the CONTRACT wins over the path ────────── */
// deptOf() infers a department from where the spec file sits on disk. For a
// handful of agents the disk tree and the governed vocabulary disagree, and the
// governed vocabulary is authoritative: the decision engine routes on the
// CONTRACT's `department`, not on a docs path.
//
// `director_growth` lives under CMO/Marketing/ on disk, so deptOf() returns
// "marketing". Its contract declares department="growth"
// (src/skylize/contracts/mvp/growth.py:17), the engine's department vocabulary
// lists "growth" as an event-raising department
// (src/skylize/decision_engine/constants.py:31-36), and ADR-0005 fixes that
// vocabulary. The path is preserved on disk per 00_organization_chart.md §6;
// this table is how the generated data reflects the contract instead.
//
...
for (const row of rows) {
  ...
  const dept = DEPT_OVERRIDE[id] || DEPT_OVERRIDE[diskId] || deptOf(row.filepath);
...
/* ── emit TS (website) + JS (console app) ─────────────────────────── */
...
// The console app imports the same two arrays with the TS type annotations and
// the `import type` line stripped, plus a named export at the end. Emitting it
// HERE is what keeps the two surfaces from drifting: one build, one roster.
const consoleBody =
  `// Generated from Skylize/${OUT} (types stripped)\n` +
  header.replace(/\nimport type .*\n/, "") +
  `\n\nconst DEPARTMENTS = [\n${deptLines}\n];\n` +
  `\nconst AGENTS = [\n${agents.map(agentLine).join("\n")}\n];\n` +
  `\n\nexport { DEPARTMENTS, AGENTS };\n`;

fs.mkdirSync(path.dirname(OUT), { recursive: true });
fs.writeFileSync(OUT, body, "utf8");
fs.mkdirSync(path.dirname(OUT_CONSOLE), { recursive: true });
fs.writeFileSync(OUT_CONSOLE, consoleBody, "utf8");

console.log(`Wrote ${OUT}`);
console.log(`Wrote ${OUT_CONSOLE}`);
